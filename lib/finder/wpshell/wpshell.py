...
def scan_directory_worker(url, dir_name):
    ...
    try:
        ...
        # Jika terjadi kesalahan SSL, coba dengan protokol HTTP
...
def get_cms_name(cms_dir):
    cms_mapping = {
        ...
        '/sites': 'Drupal',
        '/whmcms': 'WHMCMS'
        # Add more CMS names and directories as needed
    }
...
def scan_files(url, scan_type):
    ...
    if scan_type == "found" or scan_type == "listening":
        file_list = read_file_content('lib/file.txt')
...
def scan_files_worker(file_url):
    try:
        ...
        if response.status_code == 200:
            ...
            save_to_file(f"{file_url}", 'found_outputs/found_shell.txt')
        else:
...
def wpshell():
    create_directory('found_outputs')  # Pastikan direktori found_outputs sudah ada
    # Meminta pengguna untuk memasukkan nama file yang berisi daftar URL target
    target_file_path = input(f"{yellow}Input your target file => {reset}")

    # Membaca daftar URL target dari file
    target_urls = read_file_content(target_file_path)

    # Mulai pemindaian untuk setiap URL target
    for target_url in target_urls:
        print(f"\n===== Scanning {yellow}{target_url}{reset} =====")
        # Baca daftar direktori dari file
        dir_list = read_file_content('lib/dirwp.txt')
        # Mulai pemindaian direktori
